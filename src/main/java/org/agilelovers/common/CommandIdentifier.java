package org.agilelovers.common;

public class CommandIdentifier {
    public static final String QUESTION_COMMAND = "question";
    public static final String DELETE_PROMPT_COMMAND = "delete prompt";
    public static final String CLEAR_ALL_COMMAND = "clear all";
    public static final String SETUP_EMAIL_COMMAND = "set up email";
    public static final String CREATE_EMAIL_COMMAND = "create email";
    public static final String SEND_EMAIL_COMMAND = "send email to";
}
